<template lang="html">
    ...
    <div class="rounded-2 overflow-hidden mt-10">
      ...
      <div class="text-dark bg-table" style="--bs-bg-opacity: 0.7">
        <div
          ...
          class="p-4 bg-table"
        >
          <p class="text-center fs-5 mb-6">購物車目前沒有商品</p>
          <div
            class="w-25 mx-auto btn btn-secondary text-white secondary-hover mb-5"
          >
        ...
        <table v-else>
          <tr
            class="d-none d-md-flex row g-0 p-3"
          >
          ...
          <tr
            v-for="item in cart"
            :key="item.id"
            class="row g-0 p-3 border-top border-bottom border-white align-items-center"
          >
            ...
            <td class="col-12 col-md-3 order-3 order-md-0 mb-5 mb-md-0">
              <div class="d-flex w-75 position-relative">
                ...
                <input
                  ...
                  min="1"
                />
                <button type="button" class="quantity-btn plus border-dark text-dark">
                  +
                </button>
              </div>
            </td>
            ...
            <td class="col-1 order-2 order-md-0">
              <a href="#" class="text-dark scale-hover">
  ...
  <div class="container border-bottom border-light mt-9 mt-lg-12 pb-10 mb-10">
    <div class="row">
      <div class="col-12 col-lg-8">
        <p class="fs-5 bg-title p-3 rounded-top-2">不要錯過</p>
        <div class="p-3 border border-white bg-white opacity-75 text-dark rounded-bottom-2">
          <ul class="row row-cols-2 row-cols-sm-3">
            <li
              class="col mb-5 mb-sm-0"
              v-for="item in rnadomProducts"
              :key="item.id"
            >
...
export default {
  ...
  methods: {
    ...mapActions(cartStore, ['readCart', 'addCart', 'delCartItem']),
    // get all cart item
    async getCart() {
      await this.readCart();
    },
    async getRandomProducts() {
      ...
      this.rnadomProducts = res.data.products
